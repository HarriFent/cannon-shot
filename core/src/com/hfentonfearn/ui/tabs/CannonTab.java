...
public class CannonTab extends DockTab {
    ...
    public CannonTab(DockDialog dialog) {
        ...
        addItem(items.get(0), "Powder Bags", "You pack the gun powder into bags before the fights. This will save reload time.", 0, 200, true);
        addItem(items.get(1), "Cotton Wads", "Cotton wads burn faster and leave less residue in the cannon.", 0, 150, true);
        addItem(items.get(2), "Sponger Stick", "Investing in sponger sticks, the cannons are ready to fire sooner.", 0, 100, true);
        addItem(items.get(3), "Longer Slow Match", "Lighting the cannons has never been so easy. Fire in the hole!", 0, 75, true);
        ...
        addItem(items.get(5), "De-rust Cannons", "With a bit of oil and elbow grease, the cannons are in some what of a working order.", 0, 7.5f, false);
        addItem(items.get(6), "Sturdy Cannons", "Out with the old, in with the new. These shining cannons can fire much further.", 0, 9f, false);
        addItem(items.get(7), "Larger Powder Bags", "The bigger, the better. Why didn't the crew think of this before?", 0, 11f, false);
        addItem(items.get(8), "Steel Cannonballs", "Cannonballs crafted with more precision fit snug into the barrel creating more pressure.", 0, 14f, false);
